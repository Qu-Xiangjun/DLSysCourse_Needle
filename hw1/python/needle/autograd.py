...
class TensorOp(Op):
    ...
    def __call__(self, *args):
        # Construct tensor by make_from_op func.
...
class Value:
    ...
    op: Optional[Op]
    inputs: List["Value"] # input value list in computational graph
    ...
    def is_leaf(self):
        """Leaf node only have value, not operations to compute graph node, 
        which contains op and inputs."""
        return self.op is None

    def __del__(self):
        """Global tensor count sub 1 when delete the value."""
    ...
    def _init(
        ...
        *,
        ...
        """Class private methon to init the value of computentional node."""
        global TENSOR_COUNTER # Note the global tensor counte
        TENSOR_COUNTER += 1
    ...
    def make_const(cls, data, *, requires_grad=False):
        """Make the const value node, bot operations to compute graph node, 
        which contains op and inputs."""
    ...
    def make_from_op(cls, op: Op, inputs: List["Value"]):
        """Compute the value of computational graph by op and inpyts list."""
        value = cls.__new__(cls)
...
class Tensor(Value):
    grad: "Tensor" # The class var for back propogation gradient.
    ...
    def make_from_op(op: Op, inputs: List["Value"]):
        ...
        if not LAZY_MODE: # Compute the Tensor value drom inputs by op
    ...
    def numpy(self):
        """Return the numpy format of the tensor."""
        data = self.realize_cached_data()
        if array_api is numpy:
            return data
        return data.numpy()
